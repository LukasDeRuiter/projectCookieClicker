<svg
        width="100%"
        height="100%"
   viewBox="0 0 210 297"
   version="1.1"
   id="svg5"
   xmlns="http://www.w3.org/2000/svg"
   xmlns:svg="http://www.w3.org/2000/svg">
  <defs
     id="defs2" />
  <g
     id="layer1">
    <rect class="body1"
       style="fill:#008080;fill-rule:evenodd;stroke-width:0;stroke:#000000;stroke-miterlimit:4;stroke-dasharray:none"
       id="rect31"
       width="168.25122"
       height="143.74507"
       x="18.288176"
       y="152.15764"
       ry="10.24138" />
      <rect class="eye1"
            style="fill:#000000;fill-opacity:1;stroke:#000000;stroke-width:0;stroke-miterlimit:4;stroke-dasharray:none"
            id="rect4022-3"
            width="37.673645"
            height="35.479065"
            x="112.28941"
            y="209.94829"
            ry="6.5837326" />
    <rect class="nose1"
       style="fill:#145a48;fill-opacity:1;stroke:#000000;stroke-width:0;stroke-miterlimit:4;stroke-dasharray:none"
       id="rect1050"
       width="19.019705"
       height="55.596046"
       x="93.269699"
       y="209.94829"
       ry="6.5837326" />
    <rect class="eye1"
       style="fill:#000000;fill-opacity:1;stroke:#000000;stroke-width:0;stroke-miterlimit:4;stroke-dasharray:none"
       id="rect4022"
       width="37.673645"
       height="35.479065"
       x="55.596054"
       y="209.94829"
       ry="6.5837326" />
    <rect class="eyelid1"
       style="fill:#ffffff;fill-opacity:1;stroke:#000000;stroke-width:0;stroke-miterlimit:4;stroke-dasharray:none"
       id="rect4176"
       width="13.167488"
       height="10.24138"
       x="59.619457"
       y="213.97168"
       ry="5.1206899"
       rx="1.8288177" />
    <rect class="eyelid1"
       style="fill:#ffffff;fill-opacity:1;stroke:#000000;stroke-width:0;stroke-miterlimit:4;stroke-dasharray:none"
       id="rect4176-9"
       width="13.167488"
       height="10.24138"
       x="115.58127"
       y="214.33745"
       ry="5.1206899"
       rx="1.8288177" />
    <rect class="mouth1"
       style="fill:#000000;fill-opacity:1;stroke:#000000;stroke-width:0;stroke-miterlimit:4;stroke-dasharray:none"
       id="rect4222"
       width="96.195808"
       height="8.0467939"
       x="55.230293"
       y="273.59113"
       rx="1.8288184"
       ry="4.023397" />
      <rect class="eyebrow1"
            style="fill:#000080;stroke:#000000;stroke-width:0;stroke-miterlimit:4;stroke-dasharray:none"
            id="rect1004"
            width="137.89285"
            height="13.899012"
            x="32.91872"
            y="196.41502"
            ry="6.5837421" />
    <rect class="headwrinkle1"
       style="fill:#002e46;stroke:#000000;stroke-width:0;stroke-miterlimit:4;stroke-dasharray:none;fill-opacity:1"
       id="rect1004-1"
       width="107.90025"
       height="5.4864502"
       x="47.732147"
       y="185.44211"
       ry="2.5603461" />
    <rect class="headwrinkle1"
       style="fill:#002e46;fill-opacity:1;stroke:#000000;stroke-width:0;stroke-miterlimit:4;stroke-dasharray:none"
       id="rect1004-1-0"
       width="107.90025"
       height="5.4864502"
       x="48.097904"
       y="172.09175"
       ry="2.5603461" />
    <rect class="headwrinkle1"
       style="fill:#002e46;fill-opacity:1;stroke:#000000;stroke-width:0;stroke-miterlimit:4;stroke-dasharray:none"
       id="rect1004-1-0-6"
       width="107.90025"
       height="5.4864502"
       x="48.82943"
       y="158.5585"
       ry="2.5603461" />
  </g>
</svg>


<style>
    .eyelid1{
        animation: idle21 10s infinite alternate;
    }

    .eyebrow1, .headwrinkle1 {
        animation: idle11 5s infinite linear;
    }

    .body1 {
        animation: idle31 5s infinite linear;
    }

    @keyframes idle11 {
        0% {transform: translateY(0%);}
        50% {transform: translateY(2%);}
        100% {transform: translateY(0%);}
    }

    @keyframes idle21 {
        0% {transform: translate(0, 0);}
        10% {transform: translate(7%, 0);}
        25% {transform: translate(7%, 0);}
        35% {transform: translate(7%, 5%);}
        50% {transform: translate(7%, 5%);}
        60% {transform: translate(0, 5%);}
        75% {transform: translate(0, 5%);}
        85% {transform: translate(0, 0);}
        100% {transform: translate(0, 0);}
    }

    @keyframes idle31 {
        0% {transform: translateY(0%) scaleY(1);}
        50% {transform: translateY(5%) scaleY(0.95);}
        100% {transform: translateY(0%) scaleY(1);}
    }

</style>